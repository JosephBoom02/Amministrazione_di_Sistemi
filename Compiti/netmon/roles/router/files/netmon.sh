...
tcpdump -i eth1 -nl tcp and src net 10.1.1.0/24 and dst net 10.9.9.0/24 and dst port 22 and '( tcp[tcpflags] & (tcp-syn|tcp-fin) != 0 )' | while read TS IP SRC DIR DST FL FLAG RESTO ; do
	SIP=$(cut -f1-4 -d. <<< $SRC)
	DIP=$(cut -f1-4 -d. <<< $DST)
	SPT=$(cut -f5 -d. <<< $SRC)
